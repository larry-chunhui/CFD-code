...
void interpolation_les(variables *var, interpolated *inter, parameters *par)
{
	...
	// so far only cn:[start-1->end+1][0->ny][0->nz-1] are filled
	// To get dTxy/dy, Txy values at y = -1, ny+1 needed (by interpolation)
	// done inside Tij_fill_cn() function
	...
	// so far only ued:[start-1->end+1][0->ny-1][0->nz-1] are filled
	// no further filling is needed to get derivatives of those (DONE)
	...
	// so as to get dTyz/dy, Tyz at y = -1, ny+1 needed (by GPHOST CELL)
	// done inside Tij_fill_cn() function and interpolated already
	
	
	
	
	// Tij defined at center; Txx, Tyy, Tzz
	// this subroutines fills only wp part of ct
	// i.e. [start->end-1][0->ny-1][0, nz-1]
	...
	// so as to get dTxx/dx, Txx velues
	// at i = start-2, start-1, end are also needed (from neighboring process)
	Txx_fill_ct(inter->Txx_ct, par, 301);
    
	// so as to get dTyy/dy, Tyy velues
	// at j = -1, ny are also needed (by GPHOST CELL)
	Tyy_fill_ct(var->Tyy, inter->Tyy_ct, par);
    
	// so as to get dTzz/dz, no further Tzz velues
	// are needed (DONE)
...
void interpolation_les__2(variables *var, interpolated *inter, parameters *par)
{
	...
	// so far only cn:[start->end-1][0->ny][0->nz-1] are filled
	...
	// so far only cn:[start->end][0->ny][0->nz-1] are filled
	
	// Tij defined at ued; Tzx
	interpolate_y1_cn2ued__2(var->Tzx, inter->Tzx_ued, par);
	
	// so far only ued:[start->end][0->ny-1][0->nz-1] are filled
	// no further filling is needed to get derivatives of those (DONE)
	...
	// so as to get dTxx/dx, Txx velues
	// at i = start-1, end are also needed (from neighboring process)
	Txx_fill_ct__2(inter->Txx_ct, par, 301);
	
	// so as to get dTyy/dy, Tyy velues
	// at i = start are also needed (from neighboring process)
	
	// so as to get dTzz/dz, Tzz velues
	// at i = start are also needed (from neighboring process)
...
void interpolate_Tij_ct__4(double *Tij_cn, double *Tij_ct, parameters *par)
{
	...
	// full index_wp are filled
	interpolate_cn2wp__4(Tij_cn, scratch, par);
	
	// this subroutines fills only wp part of ct so far
...
// originally Tij calculated in index_cn[i][j][k] form
// but only i: start -> end-1 are filled.
// By fetching data from neighboring process to fill the blanks
// i.e. at i = start-1, end, end+1
// for later interpolations
// checked 2009/07/14
void Tij_fill_cn(double *Tij, parameters *par, int tag)
{
	...
	// so far only cn:[start-1->end+1][0->ny][0->nz-1] are filled
	// Tij values at y = -1, ny+1 needed for derivative, interpolation
	// values are added by interpolation
...
// this subroutines fills only wp part of ct
// so as to get dTxx/dx, Txx velues (2nd order scheme)
// at i = start, start-1, end are also needed (from neighboring process)
...
// so as to get dTyy/dy, Tyy velues
// at j = -1, ny are also needed (by GPHOST CELL same as u and w)
// checked 2009/07/29
