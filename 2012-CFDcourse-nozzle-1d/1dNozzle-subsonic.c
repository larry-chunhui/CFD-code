...
{
	FILE *fout1,*fout2;
	fout1=fopen("d:\\亚音残差.txt","w");//将数据输出到亚音残差.txt
	fout2=fopen("d:\\亚音参数.txt","w");//将数据输出到亚音参数.txt
 ...
 //b为密度，M为马赫数,A为面积函数值，AA为面积倒数值//
	...
	for (i=0;i<W;i++)
	{
		UU1[i]=U1[1]-T*(F1[1]-F1[0])/X+H1[1]*T+a*T*(U1[2]-2*U1[1]+U1[0])/X/X;
		UU2[i]=U2[1]-T*(F2[1]-F2[0])/X+H2[1]*T+a*T*(U2[2]-2*U2[1]+U2[0])/X/X;
		UU3[i]=U3[1]-T*(F3[1]-F3[0])/X+H3[1]*T+a*T*(U3[2]-2*U3[1]+U3[0])/X/X;
		FF1[i]=UU2[1];
